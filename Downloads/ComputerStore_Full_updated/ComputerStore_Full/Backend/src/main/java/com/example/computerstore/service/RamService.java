...
public class RamService {
    ...
    public Ram create(Ram ram) {
        ValidationUtil.requireNotBlank(ram.getRamName(), "tên RAM");
        if (nameExists(ram.getRamName(), null)) {
            throw new IllegalArgumentException("Tên RAM đã tồn tại, vui lòng chọn tên khác!");
        }
    ...
    public Ram update(Long id, Ram ram) {
        ...
        if (existing.isPresent()) {
            ValidationUtil.requireNotBlank(ram.getRamName(), "tên RAM");
            if (nameExists(ram.getRamName(), id)) {
                throw new IllegalArgumentException("Tên RAM đã tồn tại, vui lòng chọn tên khác!");
            }
            Ram r = existing.get();
    ...
    private boolean nameExists(String name, Long excludeId) {
        return repository.findAll().stream()
                .anyMatch(r -> r.getRamName() != null
                        && r.getRamName().trim().equalsIgnoreCase(name.trim())
