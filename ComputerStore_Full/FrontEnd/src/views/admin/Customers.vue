<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h4 class="fw-bold mb-0">Quản lý tài khoản khách hàng</h4>
    ...
    <table class="table align-middle bg-white">
      ...
      <tbody>
        ...
        <tr v-if="pagedItems.length === 0">
          <td colspan="6" class="text-center text-muted py-4">Chưa có khách hàng nào.</td>
        </tr>
      </tbody>
    </table>
      ...
      <div class="modal-box" style="width: 460px">
        ...
        <div class="d-flex justify-content-end gap-2 mt-3">
          ...
          <button class="btn btn-dark" :disabled="saving" @click="save">
            {{ saving ? "Đang thêm..." : "Thêm khách hàng" }}
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    items() {
      return this.allItems.filter((u) => (u.role || "").toUpperCase() === "USER");
    },
    pagedItems() {
  ...
  methods: {
    ...
    async save() {
      ...
      this.saving = true;
      try {
        await createUser({ ...this.form, role: "USER" });
