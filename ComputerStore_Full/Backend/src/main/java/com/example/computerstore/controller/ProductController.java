...
public class ProductController {
    ...
    /**
     * Deletes a product.
     * mode=hide  (default) -> soft delete, product is hidden from the storefront but kept in history.
     * mode=hard  -> permanently removes the row. Fails with 409 if the product is referenced by an order.
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> delete(@PathVariable Long id,
                                        @RequestParam(value = "mode", required = false, defaultValue = "hide") String mode) {
    ...
    public ResponseEntity<?> createWithImage(
        ...
        Path path = new java.io.File(com.example.computerstore.config.UploadUtil.getUploadsDir(), fileName).toPath();
