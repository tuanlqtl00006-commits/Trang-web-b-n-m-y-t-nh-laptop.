...
public class CpuService {
    ...
    public Cpu create(Cpu cpu) {
        ValidationUtil.requireNotBlank(cpu.getCpuName(), "tên CPU");
        if (nameExists(cpu.getCpuName(), null)) {
            throw new IllegalArgumentException("Tên CPU đã tồn tại, vui lòng chọn tên khác!");
        }
    ...
    public Cpu update(Long id, Cpu cpu) {
        ...
        if (existing.isPresent()) {
            ValidationUtil.requireNotBlank(cpu.getCpuName(), "tên CPU");
            if (nameExists(cpu.getCpuName(), id)) {
                throw new IllegalArgumentException("Tên CPU đã tồn tại, vui lòng chọn tên khác!");
            }
            Cpu c = existing.get();
    ...
    private boolean nameExists(String name, Long excludeId) {
        return repository.findAll().stream()
                .anyMatch(c -> c.getCpuName() != null
                        && c.getCpuName().trim().equalsIgnoreCase(name.trim())
